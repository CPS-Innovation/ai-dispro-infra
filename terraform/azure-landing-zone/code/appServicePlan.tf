...
resource "azurerm_private_endpoint" "fadependency-sa-file-pe" {
  name                = "pe-sa-file-aidfadep-${var.subscription}-01"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  ...
  private_service_connection {
    name                           = "psc-sa-file-aidfadep-${var.subscription}-01"
    private_connection_resource_id = azurerm_storage_account.fadependency-sa.id
    is_manual_connection           = false
  ...
  private_dns_zone_group {
    name = "pdz-sa-file-aidfadep-${var.subscription}-01"
    private_dns_zone_ids = [
      azurerm_private_dns_zone.storage_file.id
    ]
  }

  custom_network_interface_name = "nic-pe-sa-file-aidfadep-${var.subscription}-01"
