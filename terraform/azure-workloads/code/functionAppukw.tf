resource "azurerm_linux_function_app" "aid_func_2" {
  name                          = "fa-aid-${var.environment}-02"
  resource_group_name           = "rg-aid-app-workloads-02"
  location                      = "ukwest"
  service_plan_id               = data.azurerm_app_service_plan.shared_asp_2.id
  storage_account_name          = data.azurerm_storage_account.fadependency_sa_2.name
  storage_account_access_key    = data.azurerm_storage_account.fadependency_sa_2.primary_access_key
  virtual_network_subnet_id     = data.azurerm_subnet.asp_shrd_vnetint_subnet_2.id
  ...
  app_settings = {
    ...
    WEBSITE_CONTENTAZUREFILECONNECTIONSTRING = data.azurerm_storage_account.fadependency_sa_2.primary_connection_string
...
resource "azurerm_private_endpoint" "func_pe" {
  name                = "pe-fa-aid-${var.environment}-02"
  location            = "ukwest"
  resource_group_name = "rg-aid-app-workloads-02"
  subnet_id           = data.azurerm_subnet.pe_subnet_2.id
  tags                = module.tags.keyvalues

  private_service_connection {
    name                           = "psc-fa-aid-${var.environment}-01"
    private_connection_resource_id = azurerm_linux_function_app.aid_func_2.id
    is_manual_connection           = false
    subresource_names              = ["sites"]
  }

  private_dns_zone_group {
    name = "pdz-fa-${var.environment}-02"
    private_dns_zone_ids = [
      data.azurerm_private_dns_zone.app.id
    ]
  }

  custom_network_interface_name = "nic-pe-fa-aid-${var.environment}-02"

  depends_on = [azurerm_linux_function_app.aid_func_2]
}